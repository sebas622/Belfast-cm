...
function PresupuestoView({ tipo, setView }) {
    ...
    useEffect(() => { (async () => { try { const r = await window.storage.get(key); if (r?.value) setItems(JSON.parse(r.value)); } catch { } setLoaded(true); })(); }, []);
    useEffect(() => { if (loaded) window.storage.set(key, JSON.stringify(items)).catch(() => { }); }, [items, loaded]);
...
function PanelVigilancia({ setView }) {
    ...
    useEffect(() => { (async () => { try { const r = await window.storage.get('bcm_camaras'); if (r?.value) setCamaras(JSON.parse(r.value)); } catch { } setLoaded(true); })(); }, []);
    useEffect(() => { if (loaded) window.storage.set('bcm_camaras', JSON.stringify(camaras)).catch(() => { }); }, [camaras, loaded]);
...
function Presentismo({ personal, setView }) {
    ...
    useEffect(() => { (async () => { try { const r = await window.storage.get('bcm_presentismo'); if (r?.value) { const d = JSON.parse(r.value); setRegistros(d.registros || {}); setBioLink(d.bioLink || ''); } } catch { } setLoaded(true); })(); }, []);
    useEffect(() => { if (loaded) window.storage.set('bcm_presentismo', JSON.stringify({ registros, bioLink })).catch(() => { }); }, [registros, bioLink, loaded]);
...
function InformesIngeniero({ setView }) {
    ...
    useEffect(() => { (async () => { try { const r = await window.storage.get('bcm_inf_ing'); if (r?.value) setInformes(JSON.parse(r.value)); } catch { } setLoaded(true); })(); }, []);
    useEffect(() => { if (loaded) window.storage.set('bcm_inf_ing', JSON.stringify(informes)).catch(() => { }); }, [informes, loaded]);
...
function InformesIA({ obras, personal, lics, alerts, setView, apiKey, cfg }) {
    ...
    useEffect(() => {
        (async () => {
            try {
                const r = await window.storage.get('bcm_inf_ia'); if (r?.value) { const d = JSON.parse(r.value); setInformes(d.informes || []); setModeloDiario(d.modeloDiario || ''); setModeloSemanal(d.modeloSemanal || ''); }
            } catch { } setLoaded(true);
        })();
    }, []);
    useEffect(() => { if (loaded) window.storage.set('bcm_inf_ia', JSON.stringify({ informes, modeloDiario, modeloSemanal })).catch(() => { }); }, [informes, modeloDiario, modeloSemanal, loaded]);
...
function Chat({ contactos, lics, obras, personal, alerts, msgs, setMsgs, cfg, apiKey }) {
    ...
    useEffect(() => {
        (async () => {
            try { const r = await window.storage.get('bcm_username'); if (r?.value) setUserName(r.value); } catch { }
            setNameLoaded(true);
        })();
    }, []);
    ...
    async function buildContext() {
        ...
        try { const r1 = await window.storage.get('bcm_presup_materiales'); if (r1?.value) pm = JSON.parse(r1.value); const r2 = await window.storage.get('bcm_presup_subcontratos'); if (r2?.value) ps = JSON.parse(r2.value); const r3 = await window.storage.get('bcm_camaras'); if (r3?.value) camaras = JSON.parse(r3.value); const r4 = await window.storage.get('bcm_presentismo'); if (r4?.value) pres = JSON.parse(r4.value); } catch { }
        const today = new Date().toLocaleDateString('es-AR');
    ...
    const WelcomeScreen = () => {
        ...
        if (userName) {
            return (<div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 24px 0" }}>
                ...
                <button onClick={() => { setUserName(''); window.storage.set('bcm_username', '').catch(() => { }); }} style={{ marginTop: 18, fontSize: 11, color: T.muted, background: "none", border: "none", cursor: "pointer", textDecoration: "underline" }}>No soy {userName}</button>
            </div>);
        }
...
export default function App() {
    ...
    useEffect(() => {
        (async () => {
            try { const r = await window.storage.get('bcm_cfg_locked'); if (r?.value === 'true') setCfgLocked(true); } catch { }
    ...
    useEffect(() => {
        if (!dataLoaded) return;
        window.storage.set("bcm_state", JSON.stringify({ lics, obras, personal, alerts })).catch(() => { });
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const rk = await window.storage.get('bcm_apikey').catch(() => null);
